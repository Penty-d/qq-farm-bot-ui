  ...
  if (analyticsSortBy) {
    try {
      ...
      for (const row of rankings) {
        ...
        if (Number.isFinite(lv) && lv > state.level)
          continue
  ...
  try {
    ...
    for (const land of landsReply.lands) {
      ...
      if (phaseVal === PlantPhase.MATURE)
        ...
        landStatus = 'dead'
      else if (phaseVal === PlantPhase.UNKNOWN || !plant.phases.length)
        landStatus = 'empty'
...
async function runFarmOperation(opType) {
  ...
  if (status.needBug.length)
    statusParts.push(`虫:${status.needBug.length}`)
