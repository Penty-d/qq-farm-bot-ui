...
function canGetExp(opId) {
  ...
  if (!limit)
    return false // 没有限制信息，保守起见不帮助（等待农场检查获取限制）
...
function analyzeFriendLands(lands, myGid, friendName = '') {
  ...
  for (const land of lands) {
    ...
    if (!plant || !plant.phases || plant.phases.length === 0) {
      if (showDebug)
        console.log(`  [${friendName}] 土地#${id}: 无植物或无阶段数据`)
      continue
    }
    ...
    if (!currentPhase) {
      if (showDebug)
        console.log(`  [${friendName}] 土地#${id}: getCurrentPhase返回null`)
      continue
    }
    ...
    if (plant.weed_owners && plant.weed_owners.length > 0)
      result.needWeed.push(id)
    if (plant.insect_owners && plant.insect_owners.length > 0)
...
async function getFriendLandsDetail(friendGid) {
  try {
    ...
    for (const land of lands) {
      ...
      if (phaseVal === PlantPhase.MATURE)
        landStatus = plant.stealable ? 'stealable' : 'harvested'
  ...
  try {
    ...
    priorityFriends.sort((a, b) => {
      if (b.stealNum !== a.stealNum)
        return b.stealNum - a.stealNum // 偷菜优先
      // 其次按帮助需求总数
    ...
    for (let i = 0; i < friendsToVisit.length; i++) {
      ...
      try {
        ...
        // 单个好友访问失败不影响整体
        if (DEBUG_FRIEND_LANDS)
          console.log(`[调试] 访问 [${friend.name}] 异常: ${e.message}`)
      }
