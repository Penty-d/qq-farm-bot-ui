...
const { isAutomationOn, getFriendQuietHours, getFriendStealExcludeSeedIds, getFriendBlacklist } = require('../models/store');
...
async function visitFriend(friend, totalActions, myGid) {
    ...
    // 2. 偷菜操作
    const excludedSeedIds = new Set(getFriendStealExcludeSeedIds());
    const stealableTargets = excludedSeedIds.size > 0
        ? status.stealableInfo.filter(item => !excludedSeedIds.has(toNum(item.seedId)))
        : status.stealableInfo;
    if (isAutomationOn('friend_steal') && stealableTargets.length > 0) {
        const precheck = await checkCanOperateRemote(gid, 10008);
        if (precheck.canOperate) {
            const canStealNum = precheck.canStealNum > 0 ? precheck.canStealNum : stealableTargets.length;
            const targetInfos = stealableTargets.slice(0, canStealNum);
            const targetLands = targetInfos.map(item => item.landId);
            ...
            try {
                ...
                targetLands.forEach((landId) => {
                    const info = targetInfos.find(x => x.landId === landId);
...
async function checkFriends() {
    ...
    try {
        ...
        for (let i = 0; i < friendsToVisit.length; i++) {
            ...
            try {
                const result = await visitFriend(friend, totalActions, state.gid);
                visitedCount++;
